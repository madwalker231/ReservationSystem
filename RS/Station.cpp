...
int Station::fillUp()
{
	...
	/*unique_lock<mutex> lock(*stationMutex);
	int pumpIndex = -1;
	...
	for (int i = 0; i < pumpsInStation; ++i)
	{
		...
		{
			stationCondition->wait(lock);
		}
	}

	lock.unlock();
	pumps[pumpIndex].fillTankUp();

	lock.lock();
	freeMask &= ~(1 << pumpIndex);
	lock.unlock();

	stationCondition->notify_all();

	return 1;*/
	//^ fill count not +/-5. try count normal

	std::unique_lock<std::mutex> StationLock(*stationMutex);
	...
	this->stationCondition->wait(StationLock);
	//^ fill count close to -/+ 5. try count on spot
